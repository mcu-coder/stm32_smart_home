...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagfan)
    {
        bitFieldBitLen += fan_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagfan)
    {
        info->event[info->num] = EVENT_fan;
        info->num++;
        dataPoints->valuefan = gizVarlenDecompressionValue(bitFieldOffset,fan_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += fan_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuefan != cur->valuefan)
    {
        GIZWITS_LOG("valuefan Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagfan = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagfan)
    {
        gizVarlenCompressValue(bitFieldOffset,fan_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuefan);
        bitFieldOffset += fan_LEN;
    }
