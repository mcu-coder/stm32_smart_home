...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagaircondi)
    {
        bitFieldBitLen += aircondi_LEN;
    }
    if(0x01 == aFlag->flagcurtain)
    {
        bitFieldBitLen += curtain_LEN;
    }
    if(0x01 == aFlag->flaglight)
    {
        bitFieldBitLen += light_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagaircondi)
    {
        info->event[info->num] = EVENT_aircondi;
        info->num++;
        dataPoints->valueaircondi = gizVarlenDecompressionValue(bitFieldOffset,aircondi_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += aircondi_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagcurtain)
    {
        info->event[info->num] = EVENT_curtain;
        info->num++;
        dataPoints->valuecurtain = gizVarlenDecompressionValue(bitFieldOffset,curtain_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += curtain_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaglight)
    {
        info->event[info->num] = EVENT_light;
        info->num++;
        dataPoints->valuelight = gizVarlenDecompressionValue(bitFieldOffset,light_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += light_LEN;
    }


    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempGate)
    {
        info->event[info->num] = EVENT_tempGate;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempGate,(uint8_t *)&issuedData[byteFieldOffset],tempGate_LEN);
        byteFieldOffset += tempGate_LEN;
        dataPoints->valuetempGate = gizX2Y(tempGate_RATIO,  tempGate_ADDITION, elongateP0FormTmp.devStatus.valuetempGate); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaglightnessGate)
    {
        info->event[info->num] = EVENT_lightnessGate;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuelightnessGate,(uint8_t *)&issuedData[byteFieldOffset],lightnessGate_LEN);
        byteFieldOffset += lightnessGate_LEN;
        dataPoints->valuelightnessGate = gizX2Y(lightnessGate_RATIO,  lightnessGate_ADDITION, exchangeBytes(elongateP0FormTmp.devStatus.valuelightnessGate));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueaircondi != cur->valueaircondi)
    {
        GIZWITS_LOG("valueaircondi Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagaircondi = 1;
        ret = 1;
    }
    if(last->valuecurtain != cur->valuecurtain)
    {
        GIZWITS_LOG("valuecurtain Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagcurtain = 1;
        ret = 1;
    }
    ...
    if(last->valuemode != cur->valuemode)
    {
        ...
        gizwitsProtocol.waitReportDatapointFlag.flagmode = 1;
        ret = 1;
    }
    if(last->valuestatus != cur->valuestatus)
    {
        GIZWITS_LOG("valuestatus Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagstatus = 1;
        ret = 1;
    }
    if(last->valuetempGate != cur->valuetempGate)
    {
        GIZWITS_LOG("valuetempGate Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempGate = 1;
        ret = 1;
    }
    if(last->valuelightnessGate != cur->valuelightnessGate)
    {
        GIZWITS_LOG("valuelightnessGate Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaglightnessGate = 1;
        ret = 1;
    }
    ...
    if(last->valuelightness != cur->valuelightness)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuelightness Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaglightness = 1;
            ret = 1;
        }
    }
    if(last->valuemq2 != cur->valuemq2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuemq2 Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagmq2 = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagaircondi)
    {
        gizVarlenCompressValue(bitFieldOffset,aircondi_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueaircondi);
        bitFieldOffset += aircondi_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagcurtain)
    {
        gizVarlenCompressValue(bitFieldOffset,curtain_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuecurtain);
        bitFieldOffset += curtain_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaglight)
    {
        gizVarlenCompressValue(bitFieldOffset,light_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuelight);
        bitFieldOffset += light_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagmode)
    {
        ...
        {
                gizVarlenCompressValue(bitFieldOffset,mode_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuemode);
                bitFieldOffset += mode_LEN;
        }
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagstatus)
    {
        if(dataPoints->valuestatus >= status_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valuestatus Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,status_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuestatus);
                bitFieldOffset += status_LEN;
        }
    }
    ...
    {
        devStatusTmp.valuehumi = gizY2X(humi_RATIO,  humi_ADDITION, dataPoints->valuehumi); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehumi,humi_LEN);
        byteFieldOffset += humi_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempGate)
    {
        devStatusTmp.valuetempGate = gizY2X(tempGate_RATIO,  tempGate_ADDITION, dataPoints->valuetempGate); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempGate,tempGate_LEN);
        byteFieldOffset += tempGate_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flaglightness)
    {
        devStatusTmp.valuelightness = exchangeBytes(gizY2X(lightness_RATIO,  lightness_ADDITION, dataPoints->valuelightness)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuelightness,lightness_LEN);
        byteFieldOffset += lightness_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagmq2)
    {
        devStatusTmp.valuemq2 = exchangeBytes(gizY2XFloat(mq2_RATIO,  mq2_ADDITION, dataPoints->valuemq2));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuemq2,mq2_LEN);
        byteFieldOffset += mq2_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaglightnessGate)
    {
        devStatusTmp.valuelightnessGate = exchangeBytes(gizY2X(lightnessGate_RATIO,  lightnessGate_ADDITION, dataPoints->valuelightnessGate)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuelightnessGate,lightnessGate_LEN);
        byteFieldOffset += lightnessGate_LEN;
    }
